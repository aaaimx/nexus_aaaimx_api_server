import {
  DeleteEventUseCase,
  DeleteEventInput,
} from "@/application/use-cases/event/delete-event.use-case";
import { IEventRepository } from "@/domain/repositories/event.repository";
import { Event } from "@/domain/entities/event.entity";
import AppException from "@/shared/utils/exception.util";

describe("DeleteEventUseCase", () => {
  let deleteEventUseCase: DeleteEventUseCase;
  let mockEventRepository: jest.Mocked<IEventRepository>;

  const mockEvent: Event = {
    id: "event-123",
    name: "Test Event",
    description: "Test event description",
    status: "PUBLISHED",
    eventType: "SINGLE",
    startDate: new Date("2025-12-25"),
    endDate: new Date("2025-12-25"),
    isRecurring: false,
    sessionDurationMinutes: 480,
    startTime: "09:00",
    endTime: "17:00",
    location: "Test Location",
    isPublic: true,
    maxParticipants: 50,
    createdAt: new Date("2024-01-01"),
    updatedAt: new Date("2024-01-01"),
    organizerType: "USER",
    organizerUserId: "user-123",
    userId: "user-123",
  };

  beforeEach(() => {
    // Create mock repository
    mockEventRepository = {
      findById: jest.fn(),
      findAll: jest.fn(),
      create: jest.fn(),
      update: jest.fn(),
      delete: jest.fn(),
      findByUserId: jest.fn(),
      findByStatus: jest.fn(),
      findByEventType: jest.fn(),
      findByOrganizer: jest.fn(),
      findPublicEvents: jest.fn(),
      findUpcomingEvents: jest.fn(),
      findEventsByDateRange: jest.fn(),
      findByUserIdWithPagination: jest.fn(),
      findByStatusWithPagination: jest.fn(),
      findByEventTypeWithPagination: jest.fn(),
      findByOrganizerWithPagination: jest.fn(),
      findPublicEventsWithPagination: jest.fn(),
      findUpcomingEventsWithPagination: jest.fn(),
      findEventsByDateRangeWithPagination: jest.fn(),
      findAllWithPagination: jest.fn(),
      findConflictingEvents: jest.fn(),
      createEventSession: jest.fn(),
      findEventSessions: jest.fn(),
      updateEventSession: jest.fn(),
      deleteEventSession: jest.fn(),
      createEventAttendee: jest.fn(),
      findEventAttendees: jest.fn(),
      findUserEventAttendance: jest.fn(),
      updateEventAttendee: jest.fn(),
      deleteEventAttendee: jest.fn(),
      findUserEventAttendances: jest.fn(),
      countEventAttendees: jest.fn(),
      findEventsWithAvailableSlots: jest.fn(),
    };

    // Create use case instance
    deleteEventUseCase = new DeleteEventUseCase(mockEventRepository);
  });

  afterEach(() => {
    jest.clearAllMocks();
  });

  describe("execute", () => {
    it("should delete an event successfully", async () => {
      // Arrange
      const input: DeleteEventInput = {
        eventId: "event-123",
        userId: "user-123",
      };

      mockEventRepository.findById.mockResolvedValue(mockEvent);
      mockEventRepository.delete.mockResolvedValue(undefined);

      // Act
      const result = await deleteEventUseCase.execute(input);

      // Assert
      expect(mockEventRepository.findById).toHaveBeenCalledWith("event-123");
      expect(mockEventRepository.delete).toHaveBeenCalledWith("event-123");
      expect(result).toEqual({
        message: "Event deleted successfully",
      });
    });

    it("should throw AppException when event is not found", async () => {
      // Arrange
      const input: DeleteEventInput = {
        eventId: "non-existent-event",
        userId: "user-123",
      };

      mockEventRepository.findById.mockResolvedValue(null);

      // Act & Assert
      await expect(deleteEventUseCase.execute(input)).rejects.toThrow(
        new AppException("Event not found", 404)
      );
      expect(mockEventRepository.delete).not.toHaveBeenCalled();
    });

    it("should throw AppException when repository findById fails", async () => {
      // Arrange
      const input: DeleteEventInput = {
        eventId: "event-123",
        userId: "user-123",
      };

      const findError = new Error("Database connection failed");
      mockEventRepository.findById.mockRejectedValue(findError);

      // Act & Assert
      await expect(deleteEventUseCase.execute(input)).rejects.toThrow(
        new AppException(
          "Error deleting event: Database connection failed",
          500
        )
      );
    });

    it("should throw AppException when repository delete fails", async () => {
      // Arrange
      const input: DeleteEventInput = {
        eventId: "event-123",
        userId: "user-123",
      };

      mockEventRepository.findById.mockResolvedValue(mockEvent);
      const deleteError = new Error("Database delete failed");
      mockEventRepository.delete.mockRejectedValue(deleteError);

      // Act & Assert
      await expect(deleteEventUseCase.execute(input)).rejects.toThrow(
        new AppException("Error deleting event: Database delete failed", 500)
      );
    });

    it("should handle unexpected errors", async () => {
      // Arrange
      const input: DeleteEventInput = {
        eventId: "event-123",
        userId: "user-123",
      };

      const unexpectedError = new Error("Unexpected error");
      mockEventRepository.findById.mockRejectedValue(unexpectedError);

      // Act & Assert
      await expect(deleteEventUseCase.execute(input)).rejects.toThrow(
        new AppException("Error deleting event: Unexpected error", 500)
      );
    });

    it("should delete a recurring event successfully", async () => {
      // Arrange
      const recurringEvent: Event = {
        ...mockEvent,
        eventType: "RECURRING",
        isRecurring: true,
        recurrencePattern: "WEEKLY",
        recurrenceInterval: 2,
        recurrenceStartDate: new Date("2025-12-25"),
        recurrenceEndDate: new Date("2026-01-15"),
        recurrenceDays: "1,3,5",
      };

      const input: DeleteEventInput = {
        eventId: "recurring-event-123",
        userId: "user-123",
      };

      mockEventRepository.findById.mockResolvedValue(recurringEvent);
      mockEventRepository.delete.mockResolvedValue(undefined);

      // Act
      const result = await deleteEventUseCase.execute(input);

      // Assert
      expect(mockEventRepository.findById).toHaveBeenCalledWith(
        "recurring-event-123"
      );
      expect(mockEventRepository.delete).toHaveBeenCalledWith(
        "recurring-event-123"
      );
      expect(result).toEqual({
        message: "Event deleted successfully",
      });
    });

    it("should delete a workshop event successfully", async () => {
      // Arrange
      const workshopEvent: Event = {
        ...mockEvent,
        eventType: "WORKSHOP",
        name: "React Workshop",
        description: "Learn React fundamentals",
      };

      const input: DeleteEventInput = {
        eventId: "workshop-event-123",
        userId: "user-123",
      };

      mockEventRepository.findById.mockResolvedValue(workshopEvent);
      mockEventRepository.delete.mockResolvedValue(undefined);

      // Act
      const result = await deleteEventUseCase.execute(input);

      // Assert
      expect(mockEventRepository.findById).toHaveBeenCalledWith(
        "workshop-event-123"
      );
      expect(mockEventRepository.delete).toHaveBeenCalledWith(
        "workshop-event-123"
      );
      expect(result).toEqual({
        message: "Event deleted successfully",
      });
    });

    it("should delete a course event successfully", async () => {
      // Arrange
      const courseEvent: Event = {
        ...mockEvent,
        eventType: "COURSE",
        name: "JavaScript Course",
        description: "Complete JavaScript course",
      };

      const input: DeleteEventInput = {
        eventId: "course-event-123",
        userId: "user-123",
      };

      mockEventRepository.findById.mockResolvedValue(courseEvent);
      mockEventRepository.delete.mockResolvedValue(undefined);

      // Act
      const result = await deleteEventUseCase.execute(input);

      // Assert
      expect(mockEventRepository.findById).toHaveBeenCalledWith(
        "course-event-123"
      );
      expect(mockEventRepository.delete).toHaveBeenCalledWith(
        "course-event-123"
      );
      expect(result).toEqual({
        message: "Event deleted successfully",
      });
    });

    it("should delete an event with external organizer successfully", async () => {
      // Arrange
      const externalEvent: Event = {
        ...mockEvent,
        organizerType: "EXTERNAL",
        externalOrganizerName: "Tech Conference Inc.",
      };

      const input: DeleteEventInput = {
        eventId: "external-event-123",
        userId: "user-123",
      };

      mockEventRepository.findById.mockResolvedValue(externalEvent);
      mockEventRepository.delete.mockResolvedValue(undefined);

      // Act
      const result = await deleteEventUseCase.execute(input);

      // Assert
      expect(mockEventRepository.findById).toHaveBeenCalledWith(
        "external-event-123"
      );
      expect(mockEventRepository.delete).toHaveBeenCalledWith(
        "external-event-123"
      );
      expect(result).toEqual({
        message: "Event deleted successfully",
      });
    });

    it("should delete an event with division organizer successfully", async () => {
      // Arrange
      const divisionEvent: Event = {
        ...mockEvent,
        organizerType: "DIVISION",
        organizerDivisionId: "division-123",
      };

      const input: DeleteEventInput = {
        eventId: "division-event-123",
        userId: "user-123",
      };

      mockEventRepository.findById.mockResolvedValue(divisionEvent);
      mockEventRepository.delete.mockResolvedValue(undefined);

      // Act
      const result = await deleteEventUseCase.execute(input);

      // Assert
      expect(mockEventRepository.findById).toHaveBeenCalledWith(
        "division-event-123"
      );
      expect(mockEventRepository.delete).toHaveBeenCalledWith(
        "division-event-123"
      );
      expect(result).toEqual({
        message: "Event deleted successfully",
      });
    });

    it("should delete an event with club organizer successfully", async () => {
      // Arrange
      const clubEvent: Event = {
        ...mockEvent,
        organizerType: "CLUB",
        organizerClubId: "club-123",
      };

      const input: DeleteEventInput = {
        eventId: "club-event-123",
        userId: "user-123",
      };

      mockEventRepository.findById.mockResolvedValue(clubEvent);
      mockEventRepository.delete.mockResolvedValue(undefined);

      // Act
      const result = await deleteEventUseCase.execute(input);

      // Assert
      expect(mockEventRepository.findById).toHaveBeenCalledWith(
        "club-event-123"
      );
      expect(mockEventRepository.delete).toHaveBeenCalledWith("club-event-123");
      expect(result).toEqual({
        message: "Event deleted successfully",
      });
    });

    it("should delete a draft event successfully", async () => {
      // Arrange
      const draftEvent: Event = {
        ...mockEvent,
        status: "DRAFT",
      };

      const input: DeleteEventInput = {
        eventId: "draft-event-123",
        userId: "user-123",
      };

      mockEventRepository.findById.mockResolvedValue(draftEvent);
      mockEventRepository.delete.mockResolvedValue(undefined);

      // Act
      const result = await deleteEventUseCase.execute(input);

      // Assert
      expect(mockEventRepository.findById).toHaveBeenCalledWith(
        "draft-event-123"
      );
      expect(mockEventRepository.delete).toHaveBeenCalledWith(
        "draft-event-123"
      );
      expect(result).toEqual({
        message: "Event deleted successfully",
      });
    });

    it("should delete an archived event successfully", async () => {
      // Arrange
      const archivedEvent: Event = {
        ...mockEvent,
        status: "ARCHIVED",
      };

      const input: DeleteEventInput = {
        eventId: "archived-event-123",
        userId: "user-123",
      };

      mockEventRepository.findById.mockResolvedValue(archivedEvent);
      mockEventRepository.delete.mockResolvedValue(undefined);

      // Act
      const result = await deleteEventUseCase.execute(input);

      // Assert
      expect(mockEventRepository.findById).toHaveBeenCalledWith(
        "archived-event-123"
      );
      expect(mockEventRepository.delete).toHaveBeenCalledWith(
        "archived-event-123"
      );
      expect(result).toEqual({
        message: "Event deleted successfully",
      });
    });
  });
});
